...
- **Webhooks scaffold (v0.2.0 item #14).** Caller-registered HTTP endpoints fan session lifecycle and vault credential events out to downstream systems. CRUD at `POST/GET/PATCH/DELETE /v1/webhook_endpoints` plus `GET /v1/webhook_endpoints/{id}/deliveries` for audit. Each endpoint has an HMAC secret (`whsec_<base64-32>`) returned in the create response *once* (Stripe model — rotate via PATCH with `rotate_secret: true`). Outgoing requests carry `Linchpin-Signature: t=<ts>,v1=<hex>` (HMAC-SHA256 of `f"{ts}.{payload}"`). At-least-once delivery via the new background worker that polls `webhook_deliveries WHERE status IN ('pending','failed') AND next_attempt_at <= NOW()` with `FOR UPDATE SKIP LOCKED` (multi-process safe) and exponential backoff (10s / 1m / 5m / 30m / 1h) up to 5 attempts before retiring as `exhausted`. Allowlist of fanned-out event types: `session.created/status_idle/terminated/failed/requires_action`, `vault.credential.created/updated/deleted`. Two new tables (`webhook_endpoints`, `webhook_deliveries`, Alembic `0009`). `LINCHPIN_WEBHOOKS_WORKER=false` disables the in-process worker (tests use this).
- **`Linchpin-API-Version` wire-format header — phase 1 (v0.2.0 breaking bundle).** The dated `Linchpin-API-Version: 2026-05-13` header negotiates the wire format on every request. Absent header → `v1` (legacy shapes). The middleware echoes the resolved version back on the response and rejects unknown dates with `400 unsupported_api_version`. Sessions persist the version they were created with in a new `linchpin_api_version` column (Alembic `0008`) so an in-flight event stream stays in the shape its session booted with. New input shapes accepted on this PR: `permission_policy` as a discriminated `{type: "..."}` object (alongside the v1 string), and `tools` as a `linchpin_toolset_20260512` bundle (alongside the v1 flat list). Both v2 inputs auto-normalize to the canonical v1 storage shape so no data migration is needed. **Output-side translation (v1 → v2 on read), event-shape conversion (`session.requires_action` → `session.status_idle.stop_reason`, `agent.tool_use` → `agent.custom_tool_use`), and the `Linchpin-Deprecation` request-mismatch header land in v0.2.x as the breaking-bundle phase 2 — mirroring the surface-then-enforcement split used for `limited` networking and Foundations PR4.** Active deprecations recorded in `DEPRECATIONS.md`.
- **Environment networking `limited` mode — API surface (v0.2.0 item #4).** `NetworkingConfig` widens from a flat `{type: "none"|"unrestricted"}` shape to a discriminated union including `{type: "limited", allowed_hosts: [...], allow_mcp_servers: bool, allow_package_managers: bool}`. Sessions in limited mode attach to a third pre-created Docker bridge network, `linchpin-limited`. The wire shape matches Anthropic's `limited` mode so SDKs cross-port. **Note:** v0.2.0 ships the surface and persistence only — `linchpin-limited` is an internal bridge (same denial level as `none`) in this minor; the egress proxy that turns each environment's allowlist into permitted traffic lands in v0.2.x. Mirrors the v0.2.0 Foundations PR4 split (eng-review D1).
